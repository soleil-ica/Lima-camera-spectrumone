...
//
typedef int (NI488CC * GpibNotifyCallback_t)(int, unsigned int, unsigned int, unsigned int, void *);

//
// Compatibility layer for code using the regular process globals
//
extern unsigned int ibsta;
extern unsigned int iberr;
extern unsigned int ibcnt;
extern unsigned int ibcntl;
...
// If on Windows and UNICODE is defined, default to functions accepting
// wchar_t* arguments, otherwise default to char* variants.
...
// Functions using char* arguments are available on all platforms.
int  NI488CC ibfindA  (const char * udname);
unsigned int NI488CC ibrdfA   (int ud, const char * filename);
unsigned int NI488CC ibwrtfA  (int ud, const char * filename);

#if defined(_WIN32)
// Functions accepting wchar_t* arguments are implemented only for Windows.
// The variants accepting char* arguments should be used on other platforms.
int  NI488CC ibfindW  (const wchar_t * udname);
unsigned int NI488CC ibrdfW   (int ud, const wchar_t * filename);
unsigned int NI488CC ibwrtfW  (int ud, const wchar_t * filename);
#endif

unsigned int NI488CC ibask    (int ud, int option, int * v);
unsigned int NI488CC ibcac    (int ud, int v);
unsigned int NI488CC ibclr    (int ud);
unsigned int NI488CC ibcmd    (int ud, const void * buf, size_t cnt);
unsigned int NI488CC ibcmda   (int ud, const void * buf, size_t cnt);
unsigned int NI488CC ibconfig (int ud, int option, int v);
...
unsigned int NI488CC ibgts    (int ud, int v);
unsigned int NI488CC iblck    (int ud, int v, unsigned int LockWaitTime, void * Reserved);
unsigned int NI488CC iblines  (int ud, short * result);
unsigned int NI488CC ibln     (int ud, int pad, int sad, short * listen);
unsigned int NI488CC ibloc    (int ud);
unsigned int NI488CC ibnotify (int ud, int mask, GpibNotifyCallback_t Callback, void * RefData);
unsigned int NI488CC ibonl    (int ud, int v);
unsigned int NI488CC ibpct    (int ud);
unsigned int NI488CC ibppc    (int ud, int v);
unsigned int NI488CC ibrd     (int ud, void * buf, size_t cnt);
unsigned int NI488CC ibrda    (int ud, void * buf, size_t cnt);
unsigned int NI488CC ibrpp    (int ud, char * ppr);
unsigned int NI488CC ibrsp    (int ud, char * spr);
unsigned int NI488CC ibsic    (int ud);
unsigned int NI488CC ibstop   (int ud);
unsigned int NI488CC ibtrg    (int ud);
unsigned int NI488CC ibwait   (int ud, int mask);
unsigned int NI488CC ibwrt    (int ud, const void * buf, size_t cnt);
unsigned int NI488CC ibwrta   (int ud, const void * buf, size_t cnt);
...
//
unsigned int NI488CC Ibsta(void);
unsigned int NI488CC Iberr(void);
unsigned int NI488CC Ibcnt(void);

//
// Functions to access Thread-local GPIB global variables
//
unsigned int NI488CC ThreadIbsta(void);
unsigned int NI488CC ThreadIberr(void);
unsigned int NI488CC ThreadIbcnt(void);
